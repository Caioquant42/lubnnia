...
const PairTrading: React.FC<PairTradingProps> = ({
  ...
  const [assetType, setAssetType] = useState<'stocks' | 'currencies'>(
    defaultType
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(
          `Falha ao carregar dados de cointegração de ${assetType === 'stocks' ? 'ações' : 'moedas'}. Tente novamente mais tarde.`
        );
      } finally {
        setLoading(false);
      }
  ...
  const cointegratedPairs =
    cointegrationData?.[period]?.results?.filter(
        ...
        <TabsContent value='pairs'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Analise pares cointegrados para oportunidades de arbitragem
                estatística
              </CardDescription>
              ...
              <div className='flex flex-col sm:flex-row gap-4 mt-4'>
                <Select
                  value={assetType}
                  onValueChange={(value: 'stocks' | 'currencies') =>
                ...
                <Select
                  value={period}
                  onValueChange={(value: 'last_6_months' | 'last_12_months') =>
                ...
                <div className='space-y-6'>
                  <div>
                    ...
                    <p className='text-sm text-muted-foreground mb-4'>
                      Encontrados {cointegratedPairs.length} pares cointegrados
                      de {cointegrationData?.[period]?.results?.length || 0}{' '}
                      combinações totais (
                      {cointegrationData?.[
                        period
                      ]?.summary?.cointegrated_percentage?.toFixed(2) || 0}
                      %)
                    </p>
                    ...
                    {cointegratedPairs.length > 0 ? (
                      <div className='grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'>
                        {cointegratedPairs
                          .slice(0, 9)
                          .map((pair: CointegrationItem, index: number) => (
                            <Card
                              key={index}
                              ...
                              <CardContent className='p-4'>
                                ...
                                <div className='space-y-1 text-sm text-muted-foreground'>
                                  <div className='flex justify-between'>
                                    <span>Valor P:</span>
                                    <span>
                                      {pair.p_value?.toFixed(4) || 'N/A'}
                                    </span>
                                  </div>
                                  <div className='flex justify-between'>
                                    <span>Beta:</span>
                                    <span>
                                      {pair.beta?.toFixed(3) || 'N/A'}
                                    </span>
                                  </div>
                                  <div className='flex justify-between'>
                                    <span>Meia-Vida:</span>
                      ...
                      <div className='text-center py-8'>
                        <p className='text-muted-foreground'>
                          Nenhum par cointegrado encontrado para os critérios
                          selecionados.
                        </p>
                      </div>
                    )}
                  ...
                  {selectedPair && (
                    <div className='mt-6'>
                      <h3 className='text-lg font-medium mb-4'>
                        Detalhes do Par: {selectedPair.asset1} /{' '}
                        {selectedPair.asset2}
                      </h3>
                      <PairTradingView
            ...
            <CardFooter className='flex flex-col items-start'>
              <div className='text-sm text-muted-foreground'>
                ...
                <p>
                  Pair trading é uma estratégia neutra ao mercado que combina
                  uma posição comprada com uma posição vendida em um par de
                  instrumentos altamente correlacionados. Quando a correlação
                  enfraquece (spread aumenta), a estratégia opera na suposição
                  de que os preços retornarão à sua norma estatística.
                </p>
              </div>
            </CardFooter>
          </Card>
