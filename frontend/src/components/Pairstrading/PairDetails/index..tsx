...
export function PairDetails({ data }: PairDetailsProps) {
  if (!data) {
    return (
    ...
    <div className='space-y-6'>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-muted-foreground mt-1'>
              {data.p_value !== undefined && data.p_value < 0.05
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {data.half_life !== undefined
                ? formatNumber(data.half_life, 1)
                : 'N/A'}
            </div>
            <p className='text-xs text-muted-foreground mt-1'>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {data.current_zscore !== undefined
                ? formatNumber(data.current_zscore, 2)
                : 'N/A'}
            </div>
            <p className='text-xs text-muted-foreground mt-1'>
              {data.current_zscore !== undefined &&
              Math.abs(data.current_zscore) > 2
              ...
              <div className='space-y-4'>
                ...
                {data.cointegrated && (
                  <div>
                    ...
                    <p className='text-sm text-muted-foreground'>
                      Quando o Z-Score excede 2 (sobrevalorizado) ou -2
                      (subvalorizado), considere posições contrárias: comprar o
                      ativo subvalorizado e vender o sobrevalorizado, esperando
                      que retornem à sua relação histórica.
                    </p>
                  </div>
                )}
              </div>
          ...
          <Card>
            <CardContent className='pt-6'>
              {data.recent_signals && data.recent_signals.length > 0 ? (
                    ...
                    <tbody>
                      {data.recent_signals.map((signal: any, index: number) => (
                        <tr key={index} className='border-b'>
                          ...
                          <td className='p-3'>
                            {formatNumber(signal.current_zscore)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <p className='text-muted-foreground'>
                  Nenhum sinal recente disponível para este par.
                </p>
              )}
            </CardContent>
          </Card>
