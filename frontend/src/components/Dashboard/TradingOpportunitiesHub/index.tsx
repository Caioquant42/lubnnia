...
import { ArrowLeftRight, RefreshCw, Target } from 'lucide-react';
...
export function TradingOpportunitiesHub({
  className,
}: TradingOpportunitiesHubProps) {
  ...
  const buySignals = recentSignals.filter(
    (signal) => signal.signal_type === 'buy'
  ).length;
  const sellSignals = recentSignals.filter(
    (signal) => signal.signal_type === 'sell'
  ).length;
    ...
    return (
      <Card className={className}>
        <CardHeader>
          ...
          <CardDescription>
            Active pairs trading signals and opportunities
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <div>
          ...
          <div className='space-y-2'>
            {recentSignals.slice(0, 3).map((signal, index) => (
              <div
                key={`${signal.asset1}-${signal.asset2}-${index}`}
              ...
              >
                <div className='flex items-center gap-2'>
                  ...
                  <Badge
                    variant={
                      signal.signal_type === 'buy' ? 'default' : 'destructive'
                    }
                    className='text-xs'
                  >
