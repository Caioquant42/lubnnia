  ...
  useEffect(() => {
    const fetchIntelligenceData = async () => {
      try {
        ...
        rsiOverbought
          .slice(0, 4)
          .forEach((stock: RSIDataItem, index: number) => {
            const alertType =
              stock.RSI > 80 ? 'price_breakout' : 'volume_surge';
            intelligenceItems.push({
              ...
              title:
                alertType === 'volume_surge'
                  ? `Volume Surge: ${stock.Symbol}`
                  : `RSI Alert: ${stock.Symbol}`,
              description:
                alertType === 'volume_surge'
              ...
              change:
                stock.RSI > 70 ? Math.random() * 5 + 2 : Math.random() * 3 + 1, // Mock change percentage
            });
          });
            ...
            {intelligenceData.map((item, index) => (
              <div key={item.id}>
                <div className='flex items-start gap-3'>
                  <div
                    className={`p-1 rounded-full ${getSeverityColor(item.severity)}`}
                  >
                ...
                {index < intelligenceData.length - 1 && (
