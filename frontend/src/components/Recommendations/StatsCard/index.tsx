...
export function RecommendationsStatsCard({
  data,
  title = 'Estatísticas',
  className,
}: StatsCardProps) {
  ...
  const recommendationCounts = data.reduce(
  ...
  const returnsData = data.filter(
    (item: any) => typeof item.return_target_consensus === 'number'
  );
  const averageReturn =
    returnsData.length > 0
      ? returnsData.reduce(
          (sum: number, item: any) => sum + item.return_target_consensus,
          0
        ) / returnsData.length
      : null;
  ...
  const positiveReturnRate =
    totalRecommendations > 0
      ? (positiveReturns.length / totalRecommendations) * 100
      : 0;
  const negativeReturnRate =
    totalRecommendations > 0
      ? (negativeReturns.length / totalRecommendations) * 100
      : 0;

  // Calculate analyst coverage
  const totalAnalysts = data.reduce(
    (sum: number, item: any) => sum + (item.numberOfAnalystOpinions || 0),
    0
  );
  const avgAnalystsPerStock =
    totalRecommendations > 0 ? totalAnalysts / totalRecommendations : 0;
  ...
  const recommendationLabels: {
    ...
    underperform: {
      name: 'Abaixo do Mercado',
      color: 'bg-orange-100 text-orange-800 border-orange-200',
    },
    none: {
      name: 'Sem Recomendação',
      ...
      <CardContent className='space-y-6'>
          ...
          <div className='space-y-3'>
            ...
            <div className='pt-2 border-t'>
              <div className='flex justify-between items-center'>
                ...
                <span
                  className={`font-medium ${averageReturn > 0 ? 'text-green-600' : 'text-red-600'}`}
                >
        ...
        <div className='pt-2 border-t'>
          ...
          <div className='space-y-2 text-xs text-muted-foreground'>
            ...
            {recommendationCounts['strong_buy'] >
