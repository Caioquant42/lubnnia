...
export function CoveredCallStrategyDetail({
  option,
  onClose,
}: CoveredCallStrategyDetailProps) {
    ...
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Detalhes da estratégia para {option.symbol} vencendo{' '}
            {formatDate(option.block_date)}
          </p>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {formatPercent(option.cdi_relative_return)}
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          {!hasValidPremium && (
            <div className='mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg'>
              <p className='text-amber-800 text-sm'>
                ⚠️ Atenção: Esta estratégia tem um prêmio muito baixo, o que pode
                não ser atrativa para investimento.
              </p>
            </div>
          )}
              ...
              >
                ...
                <Tooltip
                  labelFormatter={(value) =>
                    `Preço da Ação: R$ ${Number(value).toFixed(2)}`
                  }
            ...
            <div className='grid grid-cols-1 gap-2 text-sm'>
              ...
              <div className='flex justify-between'>
                ...
                <span className='font-medium'>
                  {formatPercent(option.annual_return)}
                </span>
              </div>
              <div className='flex justify-between'>
                <span className='text-muted-foreground'>
                  Variação Spot para Retorno Máximo:
                </span>
                <span className='font-medium'>
          ...
          <div className='space-y-4'>
            <div>
              ...
              <p className='text-sm text-muted-foreground'>
                Esta estratégia de call coberto envolve manter ações{' '}
                {option.parent_symbol} e vender uma opção de compra com preço
                strike {formatCurrency(strikePrice)}. Você recebe{' '}
                {formatCurrency(premium)} em prêmio e limita seu ganho em{' '}
                {formatCurrency(maxProfit)} se a ação subir acima do preço
                strike.
              </p>
            </div>
            ...
            <div>
              ...
              <ul className='text-sm text-muted-foreground space-y-1 list-disc list-inside'>
                <li>
                  Ganho limitado: Os lucros são limitados se o preço da ação
                  exceder {formatCurrency(strikePrice)}
                </li>
                <li>
                  Exposição total à baixa: Você assume toda a perda se a ação
                  cair
                </li>
                <li>
                  Ponto de equilíbrio: {formatCurrency(breakEvenPrice)} (preço
                  atual menos prêmio recebido)
                </li>
                <li>
                  Perda máxima: {formatCurrency(maxLoss)} se a ação for a zero
                </li>
              </ul>
            </div>
          </div>
