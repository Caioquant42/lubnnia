  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(
          'Failed to load stock variation data. Please try again later.'
        );
      } finally {
        setLoading(false);
      }
      ...
      {sentiment && (
        <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
          <Card>
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${getSentimentColor(sentiment.sentiment)}`}
              >
                {sentiment.sentiment}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${sentiment.avgVariation >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
                {sentiment.avgVariation >= 0 ? '+' : ''}
                {sentiment.avgVariation}%
              </div>
              <p className='text-xs text-muted-foreground'>
                Market-wide average
              </p>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className='text-xs text-muted-foreground'>
                {((sentiment.positiveCount / stocks.length) * 100).toFixed(1)}%
                of market
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className='text-xs text-muted-foreground'>
                {((sentiment.negativeCount / stocks.length) * 100).toFixed(1)}%
                of market
              </p>
            </CardContent>
          </Card>
      ...
      {sunburstData && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Stock price variations by sector - size represents variation
              magnitude
            </CardDescription>
          </CardHeader>{' '}
          <CardContent className='flex justify-center'>
