 ...
 */
export function calculateNeededCapital(
  A: number,
  r: number,
  m: number
): number {
...
export function calculateRetirementPlan(
  ...
  const investment_fraction =
    params.investment_fraction ||
  ...
  for (let year = 0; year <= accumulation_years; year++) {
    ...
    if (year < accumulation_years) {
      current_value =
        current_value * (1 + accumulation_rate) + annual_contribution;
  ...
  for (let year = 0; year <= distribution_years; year++) {
    ...
    if (year < distribution_years) {
      ...
      remaining_value =
        (remaining_value - retirement_income) * (1 + distribution_rate);
