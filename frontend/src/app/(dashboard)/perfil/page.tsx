...
export default function ProfilePage() {
  ...
  const [notifications, setNotifications] = useState<NotificationSettings>({
    emailAlerts:
      user?.user_metadata?.notification_settings?.emailAlerts ?? true,
    pushNotifications:
      user?.user_metadata?.notification_settings?.pushNotifications ?? true,
    marketUpdates:
      user?.user_metadata?.notification_settings?.marketUpdates ?? true,
    portfolioAlerts:
      user?.user_metadata?.notification_settings?.portfolioAlerts ?? true,
    newsDigest: user?.user_metadata?.notification_settings?.newsDigest ?? false,
    tradingSignals:
      user?.user_metadata?.notification_settings?.tradingSignals ?? true,
  });

  // Security settings state
  const [security, setSecurity] = useState<SecuritySettings>({
    twoFactorAuth:
      user?.user_metadata?.security_settings?.twoFactorAuth ?? false,
    sessionTimeout:
      user?.user_metadata?.security_settings?.sessionTimeout ?? '30',
  ...
  const handleSecurityUpdate = async (
    ...
    try {
      const response = await profileService.updateSecuritySettings(
        user.id,
        newSettings
      );
  ...
  const accountCreatedDate = new Date(
    user?.user_metadata?.created_at || Date.now()
  );
  ...
  return (
    <div className='container mx-auto py-6 space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        ...
        <Badge
          variant='secondary'
          className='bg-finance-primary-100 text-finance-primary-800'
        >
              ...
              <div className='space-y-4'>
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      handleNotificationUpdate('portfolioAlerts', checked)
                    }
                    disabled={loading || saving}
                  />
                </div>
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      handleNotificationUpdate('newsDigest', checked)
                    }
                    disabled={loading || saving}
                  />
                </div>
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      handleNotificationUpdate('tradingSignals', checked)
                    }
                    disabled={loading || saving}
                  />
                </div>
