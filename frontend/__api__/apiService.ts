...
import { apiBaseUrl, defaultHeaders, getApiUrl, errorMessages, requestTimeout } from './config';
...
export const makeApiRequest = async (path: string, options = {}) => {
  ...
  const normalizedPath = path.replace(/^\/api\/api\//, '/api/').replace(/^\/api\//, '/');
  ...
  try {
    ...
    console.error(`API request failed: ${formattedError.message}`, formattedError);
