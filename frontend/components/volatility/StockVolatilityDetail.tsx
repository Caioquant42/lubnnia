...
import { StockVolatilityData, analyzeStockVolatility } from "@/__api__/volatilityApi";
import {
  ArrowUpDown,
  TrendingDown,
  TrendingUp,
...
export default function StockVolatilityDetail({
  stock,
}: StockVolatilityDetailProps) {
  const {
    category,
    color,
    trend,
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <div className="grid gap-1.5">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Rank de VI (1A):</span>
                  <span>{stock.iv_1y_rank?.toFixed(1) || "-"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Percentil de VI (1A):</span>
            ...
            <div className="space-y-2">
              ...
              <div className="grid gap-1.5">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Tendência de Preço:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Beta (Ibovespa):</span>
                  <span>{formatNumber(stock.beta_ibov)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Correlação (Ibovespa):</span>
                  <span>{formatNumber(stock.correl_ibov)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Desvio Padrão (1A):</span>
                  <span>{formatPercentage(stock.stdv_1y ? stock.stdv_1y * 100 : undefined)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Desvio Padrão (5D):</span>
                  <span>{formatPercentage(stock.stdv_5d ? stock.stdv_5d * 100 : undefined)}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <ul className="space-y-1.5 text-sm">
              {volatilityCharacteristics.length > 0 ? (
                ...
                <li className="flex items-start gap-2">
                  <Info className="h-4 w-4 mt-0.5 flex-shrink-0" />
                  <span>As métricas de volatilidade estão dentro dos ranges normais.</span>
                </li>
              )}
              ...
              {stock.iv_ewma_ratio > 1.1 && (
                <li className="flex items-start gap-2">
                  ...
                  <span>As opções estão relativamente caras comparadas à volatilidade histórica.</span>
                </li>
              )}
              ...
              {stock.iv_ewma_ratio < 0.9 && (
                <li className="flex items-start gap-2">
                  ...
                  <span>As opções estão relativamente baratas comparadas à volatilidade histórica.</span>
                </li>
              )}
              ...
              <li className="flex items-start gap-2 mt-2">
                ...
                <span className="text-muted-foreground">
                  Retorno Histórico 1A: {formatPercentage(stock.semi_return_1y ? stock.semi_return_1y * 100 : undefined)}
                </span>
              </li>
            </ul>
          </div>
