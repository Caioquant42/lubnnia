...
const getQuadrant = (rsRatio: number, rsMomentum: number): keyof typeof QUADRANTS => {
...
const RRGChart = ({
  ...
  const { processedData, trailData, symbolToColorMap } = useMemo(() => {
    if (!data || data.length === 0) return { processedData: [], trailData: [], symbolToColorMap: {} };

    // Group by symbol and sort by date
    const symbolGroups = data.reduce((acc, point) => {
      if (!acc[point.symbol]) {
        acc[point.symbol] = [];
      }
    ...
    const symbolToColorMap = symbols.reduce((acc, symbol, index) => {
      acc[symbol] = SYMBOL_COLORS[index % SYMBOL_COLORS.length];
      return acc;
    }, {} as Record<string, string>);

    // Process latest points for scatter plot
    const processedData = Object.entries(symbolGroups).map(([symbol, points]) => {
      // Sort by date and get the latest point
      const sortedPoints = points.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
      const latestPoint = sortedPoints[0];
      
      const quadrant = getQuadrant(latestPoint.rs_ratio, latestPoint.rs_momentum);
      ...
      return {
        ...
        size: selectedSymbols.includes(symbol) ? 8 : hoveredSymbol === symbol ? 6 : 5,
      };
    });
      ...
      <div className="absolute inset-0 pointer-events-none">
        <svg width="100%" height="100%" className="absolute inset-0">
          {trailData.map((trail) => {
            ...
            const isHighlighted = trail.isSelected || hoveredSymbol === trail.symbol;
            const opacity = isHighlighted ? 0.8 : 0.4;
            const strokeWidth = isHighlighted ? 2.5 : 1.5;
            
            // Calculate path for trail
            const pathData = trail.points.map((point, index) => {
              // This is a simplified calculation - in a real implementation,
              // you'd need to convert data coordinates to pixel coordinates
              const x = 60 + ((point.rs_ratio - xDomain[0]) / (xDomain[1] - xDomain[0])) * (width - 80);
              const y = height - 60 - ((point.rs_momentum - yDomain[0]) / (yDomain[1] - yDomain[0])) * (height - 80);
      ...
      return (
        <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-4 text-sm min-w-[200px]">
          <div className="flex items-center gap-2 mb-3">
            ...
            />
            <span className="font-semibold text-gray-900 text-base">{data.symbol}</span>
          </div>
          
          <div className="space-y-2 text-gray-600">
            <div className="flex justify-between">
              <span>RS Ratio:</span>
              <span className="font-medium text-gray-900">{data.rs_ratio?.toFixed(3) || 'N/A'}</span>
            </div>
            <div className="flex justify-between">
              <span>RS Momentum:</span>
              <span className="font-medium text-gray-900">{data.rs_momentum?.toFixed(3) || 'N/A'}</span>
            </div>
  ...
  return (
    <div className="relative bg-white rounded-lg border border-gray-200 p-4">
      {/* Chart Title */}
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-900">Relative Rotation Graph (RRG)</h3>
        <p className="text-sm text-gray-500">
          Interactive scatter plot showing relative strength and momentum with trails
          {showTrail && <span className="ml-1 text-blue-600">• Trails enabled</span>}
        </p>
      </div>
      ...
      <div className="mb-4 space-y-3">
        ...
        {showTrail && (
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span>Dashed lines show movement trail (last {trailLength} periods)</span>
          </div>
        )}
      </div>
      ...
      <div style={{ width: '100%', height: height }} className="relative">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <defs>
              {Object.entries(QUADRANTS).map(([key, quadrant]) => (
                <pattern key={key} id={`${key}Pattern`} patternUnits="userSpaceOnUse" width="100%" height="100%">
            ...
            >
              {processedData.map((entry, index) => {
                const quadrantInfo = QUADRANTS[entry.quadrant as keyof typeof QUADRANTS];
                ...
                return (
                  <Cell 
                    ...
                    stroke={isHighlighted ? "#000" : quadrantInfo?.borderColor || entry.symbolColor}
              ...
              <LabelList 
                ...
                content={(props: any) => {
                  ...
                  if (!payload || (!payload.isSelected && !payload.isHovered)) return null;
                  return (
                    <text 
                      x={x} 
                      y={y - 12} 
      ...
      <div className="mt-6 space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          {Object.entries(QUADRANTS).map(([key, quadrant]) => {
            const count = processedData.filter(d => d.quadrant === key).length;
            const percentage = processedData.length > 0 ? ((count / processedData.length) * 100).toFixed(1) : '0';
        ...
        <div className="text-center text-sm text-gray-600 bg-gray-50 rounded-lg p-3">
          <div className="font-medium">Total Symbols: {processedData.length}</div>
          <div className="text-xs mt-1">
            Click on data points to select/deselect • Hover for detailed information
