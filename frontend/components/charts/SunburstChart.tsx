...
import { getSectorMapping, getSectorShortName, getSectorIcon } from "@/utils/sectorMapping";
...
const SunburstChart: React.FC<SunburstChartProps> = ({ 
  ...
  const processDataForECharts = (node: SunburstNode): any[] => {
    ...
    const topSectors = node.children
      .sort((a, b) => {
        const aValue = a.children?.reduce((sum, child) => sum + Math.abs(child.variation || 0), 0) || 0;
        const bValue = b.children?.reduce((sum, child) => sum + Math.abs(child.variation || 0), 0) || 0;
    ...
    return topSectors.map(sectorNode => {
      ...
      const topStocks = sectorNode.children
    ...
    tooltip: {
      ...
      formatter: function (params: any) {
        if (params.data.variation !== undefined) {
          ...
          const positiveStocks = params.data.children?.filter((s: any) => s.variation > 0).length || 0;
          const negativeStocks = params.data.children?.filter((s: any) => s.variation < 0).length || 0;
