...
const RecentSignals = () => {
  ...
  const [signalsData, setSignalsData] = useState<RecentSignalsData | null>(null);
  ...
  useEffect(() => {
    const loadSignals = async () => {
      try {
        ...
        setError('Falha ao carregar sinais de trading recentes. Tente novamente mais tarde.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredSignals = signalsData?.last_5_days_signals.filter(signal => {
  ...
  const createPairKey = (asset1: string, asset2: string) => `${asset1}_${asset2}`;
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        {loading ? (
          ...
          <>
            {/* Signal Type Tabs */}
            <Tabs defaultValue="all" onValueChange={(value) => setSelectedSignalType(value as 'all' | 'buy' | 'sell')}>
                ...
                <TableBody>
                  {filteredSignals.length > 0 ? (
                    filteredSignals.map((signal, index) => {
                      const pairKey = createPairKey(signal.asset1, signal.asset2);
                      return (
                        <TableRow key={index} className={selectedPair === pairKey ? "bg-muted" : ""}>
                          ...
                          <TableCell>
                            <Badge variant={getZscoreBadgeVariant(signal.current_zscore)}>
                          ...
                          <TableCell>
                            ...
                            >
                              {selectedPair === pairKey ? 'Ocultar' : 'Detalhes'}
                            </Button>
                          </TableCell>
                        </TableRow>
                      );
              ...
              <div className="mt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium">
                    Detalhes do Par: {signalsData.pairs_data[selectedPair].asset1} / {signalsData.pairs_data[selectedPair].asset2}
                  </h3>
                  <Button 
                    variant="outline" 
                    size="sm"
                  ...
                  <CardContent>
                    <SpreadChart 
                      data={(() => {
                        ...
                        return pairData.dates.map((date: string, index: number) => ({
                          date: date,
                ...
                {/* Trading Strategy */}
                <div className="mt-4">
                  <h4 className="text-sm font-medium mb-2">Estratégia de Trading</h4>
                  <p className="text-sm">
                    {signalsData.pairs_data[selectedPair].zscore[signalsData.pairs_data[selectedPair].zscore.length - 1] < -2
      ...
      <CardFooter>
        <p className="text-xs text-muted-foreground">
          Exibindo sinais de trading dos últimos 5 dias de trading com 30 dias de dados históricos de spread.
        </p>
      </CardFooter>
    </Card>
  );
