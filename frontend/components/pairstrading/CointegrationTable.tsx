...
export default function CointegrationTable({ data, onPairSelect }: CointegrationTableProps) {
  // State for sorting
  const [sortColumn, setSortColumn] = useState<string | null>(null);
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('asc');

  if (!data || !data.cointegration?.results || data.cointegration.results.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">Nenhum par cointegrado encontrado.</p>
      </div>
    );
  }
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="cursor-pointer" onClick={() => handleSort('p_value')}>
              <div className="flex items-center">
                Valor P
                {renderSortIndicator('p_value')}
              </div>
            </TableHead>
        ...
        <TableBody>
          {pairs.map((pair, index) => (
            <TableRow key={index}>
              ...
              <TableCell>
                {pair.p_value !== undefined ? formatNumber(pair.p_value, 4) : 'N/A'}
              </TableCell>
              <TableCell className="text-right">
                {onPairSelect && pair.asset1 && pair.asset2 && pair.cointegrated && (
                  <Button 
