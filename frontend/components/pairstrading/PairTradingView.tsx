  ...
  useEffect(() => {
    const fetchPairDetails = async () => {
      try {
        ...
        setError(`Falha ao carregar detalhes do par. Tente novamente mais tarde.`);
      } finally {
        setLoading(false);
      }
    };
                ...
                <TabsContent value="price">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Evolução dos preços de {asset1} vs {asset2} ao longo do tempo
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <TabsContent value="spread">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Spread entre os preços normalizados e oportunidades de trading
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="h-[300px] flex items-center justify-center text-muted-foreground">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <h4 className="font-medium mb-2">Métricas de Cointegração</h4>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span>Valor P:</span>
                              <span className="font-medium">{pairData.p_value?.toFixed(4) || 'N/A'}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Beta:</span>
                              <span className="font-medium">{pairData.beta?.toFixed(3) || 'N/A'}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Meia-Vida:</span>
                              <span className="font-medium">{pairData.half_life ? Math.round(pairData.half_life) : 'N/A'} dias</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Z-Score Atual:</span>
                              <span className="font-medium">{pairData.current_zscore?.toFixed(2) || 'N/A'}</span>
                            </div>
                          </div>
                        </div>
                            ...
                            {pairData.current_zscore !== undefined && (
                              <div>
                                {Math.abs(pairData.current_zscore) > 2 ? (
                                  <Badge variant={pairData.current_zscore > 2 ? "destructive" : "default"}>
                                    {pairData.current_zscore > 2 ? "Sobrevalorizado" : "Subvalorizado"}
                                  </Badge>
                                ) : (
                                  <Badge variant="secondary">Dentro da Faixa Normal</Badge>
                                )}
                              </div>
                            )}
                ...
                <Card>
                  ...
                  <CardContent>
                    <Table>
                      ...
                      <TableBody>
                        {pairData.signals.slice(0, 5).map((signal: any, index: number) => (
                          <TableRow key={index}>
                            ...
                            <TableCell>
                              {signal.signal_type === 'buy' ? (
                                <span>Long {asset1} / Short {asset2}</span>
                              ) : (
                                <span>Short {asset1} / Long {asset2}</span>
                              )}
                            </TableCell>
                          </TableRow>
                        ))}
...
export default PairTradingView;
