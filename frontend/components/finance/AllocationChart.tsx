...
export default function AllocationChart({ positions, className }: AllocationChartProps) {
  ...
  if (smallPositions.length > 1) {
    const otherValue = smallPositions.reduce((sum, item) => sum + item.value, 0);
            ...
            <PieChart>
              ...
              <Legend
                ...
                formatter={(value, entry: any, index) => (
                  <span className="text-xs">{value} ({chartData[index].value.toFixed(1)}%)</span>
                )}
              />
            </PieChart>
          </ResponsiveContainer>
