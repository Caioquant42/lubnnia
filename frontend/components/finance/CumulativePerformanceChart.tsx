...
import { CalendarDays, TrendingUp, BarChart3, Percent, Target, AlertTriangle } from 'lucide-react';
import { 
  fetchCumulativePerformance, 
  getCumulativePerformanceDateRange,
  CumulativePerformanceDataPoint,
  CumulativePerformanceResponse 
...
export default function CumulativePerformanceChart({
  className,
  ...
  const calculateCumulativeReturns = useCallback((rawData: CumulativePerformanceDataPoint[], startDateForCalc: string) => {
    ...
    Object.keys(startDataPoint).forEach(key => {
      if (key !== 'date' && typeof startDataPoint[key] === 'number' && startDataPoint[key] !== null) {
    ...
    return rawData.slice(startIndex).map(dataPoint => {
      const newDataPoint: CumulativePerformanceDataPoint = { date: dataPoint.date };
      
      Object.keys(dataPoint).forEach(key => {
        if (key !== 'date' && typeof dataPoint[key] === 'number' && dataPoint[key] !== null) {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        setDateRange({ min: dateRangeData.min_date, max: dateRangeData.max_date });
        setAvailableAssets(dateRangeData.available_assets);
        
        // Calculate initial date range based on selected period
        const calculatedRange = calculateDateRange(selectedPeriod, dateRangeData.max_date);
        const initialStartDate = calculatedRange ? calculatedRange.start : dateRangeData.min_date;
        const initialEndDate = calculatedRange ? calculatedRange.end : dateRangeData.max_date;
        ...
        const cumulativeData = calculateCumulativeReturns(performanceData.data, initialStartDate);
        
        // Filter data to selected date range
        const filteredData = cumulativeData.filter(d => d.date >= initialStartDate && d.date <= initialEndDate);
        ...
        if (filteredData.length > 0) {
          ...
          selectedAssets.forEach(asset => {
            const assetData = filteredData.map(d => d[asset] as number).filter(v => v !== null && !isNaN(v));
            if (assetData.length > 0) {
              const returns = assetData.slice(1).map((value, index) => {
                ...
                if (percentageAssets.includes(asset)) {
                  ...
                  return prevValue !== 0 ? (value - prevValue) / (100 + prevValue) * 100 : 0;
                }
              });
              
              const totalReturn = assetData[assetData.length - 1];
              const volatility = returns.length > 1 ? Math.sqrt(returns.reduce((sum, r) => sum + Math.pow(r - returns.reduce((a, b) => a + b, 0) / returns.length, 2), 0) / (returns.length - 1)) : 0;
              ...
              customMetrics[asset] = {
                ...
                sharpeRatio: volatility !== 0 ? (totalReturn / (volatility * Math.sqrt(252))) : 0,
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Calculate cumulative returns based on selected start date
        const cumulativeData = calculateCumulativeReturns(performanceData.data, startDate);
        
        // Filter data to selected date range
        const filteredData = cumulativeData.filter(d => d.date >= startDate && d.date <= endDate);
  ...
  }, [selectedAssets, showMetrics, startDate, endDate, dateRange.min, calculateCumulativeReturns]);// Handle period selection
  ...
  const adjustDateRange = useCallback((startPercent: number, endPercent: number) => {
  ...
  const quickRangeShortcuts = useMemo(() => [
    { label: 'Last 30D', action: () => adjustDateRange(Math.max(0, sliderRange[1] - (30 / ((new Date(dateRange.max).getTime() - new Date(dateRange.min).getTime()) / (1000 * 60 * 60 * 24))) * 100), sliderRange[1]) },
    { label: 'Last 90D', action: () => adjustDateRange(Math.max(0, sliderRange[1] - (90 / ((new Date(dateRange.max).getTime() - new Date(dateRange.min).getTime()) / (1000 * 60 * 60 * 24))) * 100), sliderRange[1]) },
    { label: 'YTD', action: () => {
      ...
      const maxTime = new Date(dateRange.max).getTime();
      const totalRange = maxTime - minTime;
      const yearStartPercent = Math.max(0, ((yearStart.getTime() - minTime) / totalRange) * 100);
    ...
    if (newStartDate !== startDate || newEndDate !== endDate) {
      ...
      if (selectedPeriod !== 'Custom') {
        ...
        if (periodRange) {
          ...
          const expectedStartPercent = ((expectedStartTime - minTime) / totalRange) * 100;
          const expectedEndPercent = ((expectedEndTime - minTime) / totalRange) * 100;
          
          // If slider moved away from expected period position, set to custom
          if (Math.abs(sliderRange[0] - expectedStartPercent) > 1 || Math.abs(sliderRange[1] - expectedEndPercent) > 1) {
        ...
        <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg max-w-xs">
          ...
          <div className="space-y-2">
            {payload.map((entry: any, index: number) => {
              ...
              const previousValue = previousData ? (previousData[entry.dataKey] as number) : null;              const dailyChange = previousValue !== null ? 
                  ...
                  {previousData && previousValue && (
                    <div className="ml-5 text-xs">
                      <span className={`${dailyChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {dailyChange >= 0 ? '+' : ''}{dailyChange.toFixed(2)}% daily
                      </span>
                    </div>
                  )}
                </div>
  ...
  const additionalMetrics = useMemo(() => {
    ...
    selectedAssets.forEach(asset => {
      const assetData = data.map(d => d[asset]).filter(v => v !== null && v !== undefined) as number[];
      ...
      if (assetData.length > 1 && assetMetrics) {
        ...
        const sharpeRatio = assetMetrics.volatility > 0 ? excessReturn / assetMetrics.volatility : 0;
        ...
        <div className="flex flex-col gap-4">
              ...
              <CardTitle className="flex items-center gap-2">
                ...
                {startDate && endDate && (
                  <span className="block text-xs mt-1">
                    {formatDate(startDate)} - {formatDate(endDate)} (Returns from {formatDate(startDate)})
                  </span>
                )}
              </CardDescription>
            </div>
          ...
          {showControls && (
            <div className="space-y-4">
                    ...
                    <Badge
                      ...
                      style={{
                        backgroundColor: selectedAssets.includes(asset) 
                          ? ASSET_COLORS[asset as keyof typeof ASSET_COLORS] || '#6b7280'
                          : 'transparent',
                        borderColor: ASSET_COLORS[asset as keyof typeof ASSET_COLORS] || '#6b7280',
                        color: selectedAssets.includes(asset) ? 'white' : 'inherit'
                      }}
              ...
              </div>              {/* Fine-tune Date Range Slider */}
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-xs text-gray-500">
                    ...
                    <span className="text-center">
                      Available Range: {dateRange.min && dateRange.max && (
        ...
        {showMetrics && additionalMetrics && (
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              {Object.entries(additionalMetrics).map(([asset, metrics]: [string, any]) => (
                <div
                  key={asset}
                  className="p-4 border rounded-lg bg-gray-50/50 hover:bg-gray-50 transition-colors"
                  style={{ borderColor: ASSET_COLORS[asset as keyof typeof ASSET_COLORS] || '#6b7280' }}
                >
                  <div className="flex items-center gap-2 mb-3">
                    <div
                      className="w-3 h-3 rounded-full"
                      style={{ backgroundColor: ASSET_COLORS[asset as keyof typeof ASSET_COLORS] || '#6b7280' }}
                    />
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`font-medium ${metrics.total_return >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium text-gray-700">{metrics.volatility.toFixed(1)}%</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      <span>Current:</span>
                      <span className="font-medium">{metrics.current_value?.toFixed(2) || 'N/A'}</span>
                    </div>
                    ...
                    <div className="pt-1 border-t border-gray-200">
                      <div className="flex justify-between items-center text-xs">
                        ...
                        <span className="font-medium">
                          {metrics.min_value.toFixed(2)} - {metrics.max_value.toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              >
                ...
                {selectedAssets.map((asset) => (
                  <Line
                    ...
                    type="monotone"
                    dataKey={asset}
                    stroke={ASSET_COLORS[asset as keyof typeof ASSET_COLORS] || '#6b7280'}
                    strokeWidth={2.5}
                    dot={false}
                    activeDot={{ 
                      r: 5, 
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                ...
                <div className="font-medium mt-1">
                  {(() => {
                    const entries = Object.entries(additionalMetrics) as [string, any][];
                    ...
                    return (
                      <div className="flex items-center gap-2">
                        <div
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: ASSET_COLORS[best[0] as keyof typeof ASSET_COLORS] }}
                        />
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex items-center gap-2">
                        <div
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: ASSET_COLORS[safest[0] as keyof typeof ASSET_COLORS] }}
                        />
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex items-center gap-2">
                        <div
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: ASSET_COLORS[bestSharpe[0] as keyof typeof ASSET_COLORS] }}
                        />
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="flex items-center gap-2">
                        <div
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: ASSET_COLORS[minDrawdown[0] as keyof typeof ASSET_COLORS] }}
                        />
                        {minDrawdown[0]} (-{minDrawdown[1].maxDrawdown.toFixed(1)}%)
                      </div>
                    );
                  })()}
                </div>
