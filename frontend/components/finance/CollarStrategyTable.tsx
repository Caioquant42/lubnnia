...
export function CollarStrategyTable({ data, onSelectStrategy }: CollarStrategyTableProps) {
  const [sortColumn, setSortColumn] = React.useState<string>("gain_to_risk_ratio");
      ...
      <div className="flex items-center p-4 justify-between">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => setSortColumn("gain_to_risk_ratio")}>
              Ordenar por Relação Ganho/Risco
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setSortColumn("cdi_relative_return")}>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead>
              <div className="flex items-center gap-1 cursor-pointer" onClick={() => handleSort('put_strike')}>
                Strike Put
                {renderSortIndicator('put_strike')}
              </div>
            </TableHead>
            <TableHead>
              <div className="flex items-center gap-1 cursor-pointer" onClick={() => handleSort('call_premium')}>
                Prêmio Call
                {renderSortIndicator('call_premium')}
              </div>
            </TableHead>
            <TableHead>
              <div className="flex items-center gap-1 cursor-pointer" onClick={() => handleSort('put_premium')}>
                Prêmio Put
        ...
        <TableBody>
          {sortedData.map((strategy, index) => (
            <TableRow key={index}>
              <TableCell className="font-medium">{strategy.strategy.parent_symbol}</TableCell>
              <TableCell>
          ...
          {sortedData.length === 0 && (
            <TableRow>
              ...
              <div className="text-center py-8">
                <p className="text-muted-foreground">Nenhuma estratégia de collar encontrada.</p>
              </div>
            </TableCell>
            </TableRow>
          )}
