...
import { ChartDataPoint, ChartDataSeries, ChartTimePeriod } from "@/types";
import { generateChartData } from "@/data/mock-data";
...
const TIME_PERIODS: ChartTimePeriod[] = ['1D', '1W', '1M', '3M', '6M', 'YTD', '1Y', '5Y', 'MAX'];
...
export default function ChartCard({
  ...
  const [activePeriod, setActivePeriod] = useState<ChartTimePeriod>(initialTimePeriod);
  const [series, setSeries] = useState<ChartDataSeries>(
    initialSeries || generateChartData(initialTimePeriod, startValue, volatility)
  );
  
  const handlePeriodChange = (period: ChartTimePeriod) => {
    setActivePeriod(period);
    // Generate new chart data for the selected period
    setSeries(generateChartData(period, startValue, volatility));
  };
  ...
  const formatDate = (timestamp: number) => {
    ...
    return date.toLocaleDateString(undefined, {
      month: 'short',
      day: 'numeric',
      year: activePeriod !== '1D' && activePeriod !== '1W' ? 'numeric' : undefined,
    });
  };
        ...
        <div className="flex items-center">
          <div className="mr-4 flex items-baseline">
            ...
            <div className={cn(
              ...
              change >= 0 ? "text-finance-success-500" : "text-finance-danger-500"
            )}>
              {change >= 0 ? (
