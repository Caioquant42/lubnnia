    ...
    switch (type) {
      case 'return':
        ...
        chartData = data
          ...
          .sort((a, b) => (b.return_target_consensus || 0) - (a.return_target_consensus || 0))
          .slice(0, 10)
          .map(item => ({
            name: item.ticker || item.symbol || 'N/A',
            value: (item.return_target_consensus || 0) * 100,
            color: (item.return_target_consensus || 0) > 0 ? '#10b981' : '#ef4444'
          }));
        xAxisLabel = 'Ticker';
      ...
      case 'price':
        ...
        chartData = data
          ...
          .map(item => {
            const ratio = (item.targetMedianPrice / item.currentPrice - 1) * 100;
            return {
              name: item.ticker || item.symbol || 'N/A',
              value: ratio,
      ...
      case 'analysts':
        ...
        chartData = data
          ...
          .sort((a, b) => (b.numberOfAnalystOpinions || 0) - (a.numberOfAnalystOpinions || 0))
          .slice(0, 10)
          .map(item => ({
            name: item.ticker || item.symbol || 'N/A',
