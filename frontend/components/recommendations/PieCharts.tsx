...
export function RecommendationsPieChart({ data, className, translations }: RecommendationsPieChartProps) {
  ...
  useEffect(() => {
    ...
    const recommendationCounts: RecommendationCountType = data.reduce((acc, item) => {
    ...
    const chartData = Object.entries(recommendationCounts).map(([name, value]) => {
    ...
    const option = {
      ...
      tooltip: {
        ...
        formatter: function(params: any) {
          const tooltipTitle = translations?.tooltipTitle || 'Recommendation Type';
      ...
      legend: {
        ...
        formatter: function(name: string) {
          return translations?.keys?.[name] || name;
        },
        textStyle: {
          fontSize: 13,
          color: '#475569',
      ...
      series: [
        {
          ...
          emphasis: {
            ...
            label: {
              ...
              color: '#ffffff',
              formatter: function(params: any) {
                return translations?.keys?.[params.name] || params.name;
              }
            }
          },
          label: {
  ...
  return (
    <Card className={className}>
      <CardHeader>
        <CardTitle>{translations?.title || 'Analyst Recommendations'}</CardTitle>
      </CardHeader>
      <CardContent>
