      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center mb-16">
          ...
          <p className="text-lg text-slate-300 max-w-2xl mx-auto">
            Selecione o plano que melhor se adapta às suas necessidades de trading quantitativo
          </p>
        </div>
          ...
          {Object.entries(STRIPE_PRODUCTS).map(([key, product]) => (
            ...
            >
              ...
              <CardHeader className="text-center pb-6">
                <CardTitle className={`text-2xl ${key === 'ZOMMA_PRO' ? 'text-white' : 'text-slate-200'}`}>
                  {product.name}
                </CardTitle>
                <CardDescription className={`${key === 'ZOMMA_PRO' ? 'text-slate-300' : 'text-slate-400'}`}>
                  {product.description}
                </CardDescription>
              </CardHeader>
              ...
              <CardContent className="flex-1 px-6">
                <div className="text-center mb-8">
                  <span className={`text-4xl font-bold ${key === 'ZOMMA_PRO' ? 'text-orange-400' : 'text-slate-200'}`}>
                    {new Intl.NumberFormat('pt-BR', {
                      style: 'currency',
                      currency: product.currency,
                    }).format(product.price)}
                  </span>
                  <span className={`text-sm ${key === 'ZOMMA_PRO' ? 'text-slate-300' : 'text-slate-400'}`}>
                    {product.price === 0 ? '' : '/mês'}
                  </span>
                </div>
                  ...
                  {product.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-3">
                      <Check className={`h-5 w-5 mt-0.5 flex-shrink-0 ${
                        key === 'ZOMMA_PRO' ? 'text-orange-400' : 'text-green-400'
                      }`} />
                      <span className={`text-sm ${key === 'ZOMMA_PRO' ? 'text-slate-300' : 'text-slate-400'}`}>
                        {feature}
                      </span>
          ...
          <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-8">
            ...
            <div className="grid gap-6 md:grid-cols-3 text-sm">
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400 mb-2">52</div>
                <div className="text-slate-300">Sinais Ativos</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-400 mb-2">+47.4%</div>
                <div className="text-slate-300">Alpha 30D</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400 mb-2">74%</div>
                <div className="text-slate-300">Win Rate</div>
              </div>
            </div>
            <p className="text-slate-300 mt-6">
              Acesso completo a todas as ferramentas quantitativas institucionais com suporte prioritário
            </p>
          </div>
        </div>
      </div>
