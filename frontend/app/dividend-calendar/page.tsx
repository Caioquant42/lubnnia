...
import { Calendar, TrendingUp, Building2, DollarSign, Filter, Download, Search } from 'lucide-react';
import { fetchDividendCalendar, DividendData, DividendResponse, DividendFilters } from '@/__api__/dividendApi';
...
const DividendCalendarPage = () => {
  ...
  const getTypeBadgeColor = (tipo: string) => {
    switch (tipo.toUpperCase()) {
      case 'JCP': return 'bg-blue-100 text-blue-800';
      case 'DIV': return 'bg-green-100 text-green-800';
      case 'JSCP': return 'bg-purple-100 text-purple-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  };

  const exportToCsv = () => {
    const headers = ['Código', 'Tipo', 'Valor', 'Pagamento', 'Registro', 'Ex', 'Status'];
    const csvContent = [
      headers.join(','),
    ...
    link.setAttribute('download', `dividendos_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="container mx-auto p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Carregando calendário de dividendos...</p>
          </div>
        </div>
      </div>
    );
            ...
            <CardContent className="pt-6">
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  ...
                  <p className="text-2xl font-bold">{summary.companies_count}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  ...
                  <p className="text-2xl font-bold">{summary.total_value_display}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center">
                ...
                <div className="ml-2">
                  ...
                  <p className="text-2xl font-bold">{summary.status_breakdown?.upcoming || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, upcoming_days: parseInt(e.target.value) || 365 }))}
              />
            </div>
            <div>
              <Label htmlFor="tipo">Tipo</Label>              <Select value={filters.tipo || 'all'} onValueChange={(value) => setFilters(prev => ({ 
                ...prev, 
                tipo: value === 'all' ? undefined : (value as 'Dividendo' | 'JCP')
              }))}>
                <SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, min_value: parseFloat(e.target.value) || undefined }))}
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, max_value: parseFloat(e.target.value) || undefined }))}
              />
            </div>
            <div>
              <Label htmlFor="sort">Ordenação</Label>
              <Select value={filters.sort_order} onValueChange={(value: 'asc' | 'desc') => setFilters(prev => ({ ...prev, sort_order: value }))}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
      ...
      <Tabs defaultValue="timeline" className="space-y-4">
          ...
          {dividendsByDate.map(([date, dateDividends]) => (
            <Card key={date}>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  {dateDividends.length} pagamento{dateDividends.length !== 1 ? 's' : ''}
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {dateDividends.map((dividend, index) => (
                    <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                      ...
                      <div className="text-right">
                        <p className="font-bold text-green-600">{dividend.valor_display}</p>
        ...
        <TabsContent value="status" className="space-y-4">
          ...
          {groupedDividends.today.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-blue-600">Hoje ({groupedDividends.today.length})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {groupedDividends.upcoming.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-green-600">Próximos ({groupedDividends.upcoming.length})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {groupedDividends.paid.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-gray-600">Pagos ({groupedDividends.paid.length})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  <tbody>
                    {filteredDividends.map((dividend, index) => (
                      <tr key={index} className="border-b hover:bg-muted/50">
                        ...
                        <td className="p-2 text-right font-mono text-green-600">{dividend.valor_display}</td>
    ...
    <div className="border rounded-lg p-4 space-y-3">
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          <span className="font-bold text-green-600">{dividend.valor_display}</span>
        </div>
