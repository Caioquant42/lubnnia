...
import { CoveredCallOption, CoveredCallResponse } from '@/__api__/coveredcallApi';
import coveredcallApi from '@/__api__/coveredcallApi';
...
export default function CoveredCallStrategyPage() {
  ...
  const [coveredCallData, setCoveredCallData] = useState<CoveredCallResponse | null>(null);
  const [selectedStrategy, setSelectedStrategy] = useState<CoveredCallOption | null>(null);
  ...
  useEffect(() => {
    const fetchCoveredCallData = async () => {
      try {
        ...
        toast({
          title: 'Erro',
          description: 'Falha ao carregar estratégias de call coberto. Por favor, tente novamente mais tarde.',
          variant: 'destructive',
        });
      } finally {
        setLoading(false);
  ...
  const formatMaturityDate = (blockDate?: string, daysToMaturity?: number) => {
    ...
    return `${date.getDate().toString().padStart(2, '0')}/${
      ...
      date.getFullYear()} (${daysToMaturity} days)`;
  };
  ...
  if (selectedStrategy) {
    return (
      <div className="container mx-auto py-6">
        ...
        <CoveredCallStrategyDetail option={selectedStrategy} onClose={handleBackToList} />
      </div>
    );
  }
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Estratégias de call coberto com prêmios atrativos e retornos superiores ao CDI
        </p>
        ...
        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg dark:bg-blue-900/20 dark:border-blue-800">
          <p className="text-blue-800 dark:text-blue-200 text-sm">
            ℹ️ Apenas opções com prêmios de R$ 0,01 ou superior são exibidas para garantir estratégias atrativas.
          </p>
        </div>
      </div>
      ...
      {!coveredCallData ? (
        <Card>
          <CardContent className="pt-6">
            <p className="text-center text-muted-foreground">Nenhum dado de call coberto disponível</p>
          </CardContent>
        </Card>
      ) : (
        <Tabs 
          ...
          <TabsList className="grid grid-cols-4 mb-4">
            {Object.entries(maturityRangeTitles).map(([key, title]) => (
              <TabsTrigger 
                key={key} 
                value={key}
                disabled={!coveredCallData.results[key] || coveredCallData.results[key].length === 0}
              >
                {title}
              </TabsTrigger>
            ))}
              ...
              <TabsContent key={key} value={key} className="mt-0">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Mostrando {options.length} estratégias de call coberto ordenadas por retorno relativo CDI
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {options.length > 0 ? (
                      ...
                      <p className="text-center text-muted-foreground py-8">
                        Nenhuma estratégia de call coberto disponível para este período de vencimento
                      </p>
                    )}
                  </CardContent>
                </Card>
