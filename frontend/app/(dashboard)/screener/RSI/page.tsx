...
import { fetchRSIData, RSIDataItem, RSIScreenerResponse } from "@/__api__/screenerApi";
...
export default function RSIScreenerPage() {
  ...
  const renderRSIItem = (item: RSIDataItem) => (
    <div key={`${item.Symbol}-${item.Datetime}`} className="mb-4 rounded-lg border p-3">
      ...
      <div className="mt-2">
        ...
        <SimpleProgress value={item.RSI} className={getProgressColor(item.RSI)} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {timeframeData.overbought.length} ativos com possível tendência de reversão para baixo
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {timeframeData.oversold.length} ativos com possível tendência de reversão para cima
            </CardDescription>
          </CardHeader>
          <CardContent>
    ...
    <div className="container py-6">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">
          Análise de ativos sobrecomprados e sobrevendidos por Índice de Força Relativa (RSI)
        </p>
      </div>
