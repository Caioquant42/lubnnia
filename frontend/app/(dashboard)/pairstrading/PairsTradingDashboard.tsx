...
import { ChevronDown, Filter, SlidersHorizontal, RefreshCcw } from 'lucide-react';

// Import API functions
import { 
  getRecentSignals, getCointegrationData, extractUniqueSymbols, getPairDetails,
  PairsResponse, CointegrationPair, PairSignal, PairsTradingParams
...
export default function PairsTradingDashboard() {
  ...
  const [period, setPeriod] = useState<'last_6_months' | 'last_12_months'>('last_6_months');
  ...
  const [loadingCointegration, setLoadingCointegration] = useState<boolean>(true);
  ...
  const [cointegrationData, setCointegrationData] = useState<PairsResponse | null>(null);
    ...
    if (selectedPair) {
      const fetchPairDetails = async () => {
        try {
          ...
          const data = await getPairDetails(selectedPair.asset1, selectedPair.asset2);
      ...
      {selectedPair ? (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <div>
              <CardTitle>Análise do Par: {selectedPair.asset1} / {selectedPair.asset2}</CardTitle>
              <CardDescription>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-2 md:space-y-0 pb-4">
            <TabsList>
              ...
              <TabsTrigger value="cointegration">Pares Cointegrados</TabsTrigger>
            </TabsList>
              ...
              <Popover open={filtersVisible} onOpenChange={setFiltersVisible}>
                <PopoverTrigger asChild>
                  <Button variant="outline" className="flex items-center gap-2">
                    ...
                    <span>Filtros</span>
                    {Object.keys(filters).filter(k => k !== 'sort_by' && k !== 'sort_order' && k !== 'limit' && k !== 'cointegrated_only' && filters[k as keyof PairsTradingParams]).length > 0 && (
                      <Badge variant="secondary" className="ml-1">
                        {Object.keys(filters).filter(k => k !== 'sort_by' && k !== 'sort_order' && k !== 'limit' && k !== 'cointegrated_only' && filters[k as keyof PairsTradingParams]).length}
                      </Badge>
                    )}
                  </Button>
                </PopoverTrigger>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => handleFilterChange('asset1', value === "" ? undefined : value)}
                      >
                    ...
                    {activeTab === "signals" && (
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) => handleFilterChange('signal_type', value === "" ? undefined : value)}
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="buy">Sinais de compra</SelectItem>
                            <SelectItem value="sell">Sinais de venda</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                    )}
                          ...
                          <div className="space-y-4">
                            <div className="space-y-2">
                              <div className="flex justify-between">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {filters.min_zscore !== undefined ? filters.min_zscore : 'Qualquer'} - {filters.max_zscore !== undefined ? filters.max_zscore : 'Qualquer'}
                                </div>
                              </div>
                              <div className="flex items-center gap-4">
                                <Input 
                                  ...
                                  placeholder="Mín" 
                                  className="w-20"
                                  value={filters.min_zscore !== undefined ? filters.min_zscore : ''}
                                  onChange={(e) => handleFilterChange('min_zscore', e.target.value === '' ? undefined : parseFloat(e.target.value))}
                                />
                                <Input 
                                  type="number" 
                                  placeholder="Máx" 
                                  className="w-20"
                                  value={filters.max_zscore !== undefined ? filters.max_zscore : ''}
                                  onChange={(e) => handleFilterChange('max_zscore', e.target.value === '' ? undefined : parseFloat(e.target.value))}
                                />
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              <div className="flex justify-between">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {filters.min_beta !== undefined ? filters.min_beta : 'Qualquer'} - {filters.max_beta !== undefined ? filters.max_beta : 'Qualquer'}
                                </div>
                              </div>
                              <div className="flex items-center gap-4">
                                <Input 
                                  ...
                                  placeholder="Mín" 
                                  className="w-20"
                                  value={filters.min_beta !== undefined ? filters.min_beta : ''}
                                  onChange={(e) => handleFilterChange('min_beta', e.target.value === '' ? undefined : parseFloat(e.target.value))}
                                />
                                <Input 
                                  type="number" 
                                  placeholder="Máx" 
                                  className="w-20"
                                  value={filters.max_beta !== undefined ? filters.max_beta : ''}
                                  onChange={(e) => handleFilterChange('max_beta', e.target.value === '' ? undefined : parseFloat(e.target.value))}
                                />
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              <div className="flex justify-between">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {filters.min_half_life !== undefined ? filters.min_half_life : 'Qualquer'} - {filters.max_half_life !== undefined ? filters.max_half_life : 'Qualquer'}
                                </div>
                              </div>
                              <div className="flex items-center gap-4">
                                <Input 
                                  ...
                                  placeholder="Mín" 
                                  className="w-20"
                                  value={filters.min_half_life !== undefined ? filters.min_half_life : ''}
                                  onChange={(e) => handleFilterChange('min_half_life', e.target.value === '' ? undefined : parseFloat(e.target.value))}
                                />
                                <Input 
                                  type="number" 
                                  placeholder="Máx" 
                                  className="w-20"
                                  value={filters.max_half_life !== undefined ? filters.max_half_life : ''}
                                  onChange={(e) => handleFilterChange('max_half_life', e.target.value === '' ? undefined : parseFloat(e.target.value))}
                                />
                              </div>
                            </div>
                            ...
                            {activeTab === "cointegration" && (
                              <div className="flex items-center space-x-2 pt-2">
                                <Checkbox 
                                  ...
                                  onCheckedChange={(checked) => handleFilterChange('cointegrated_only', checked)}
                                />
                                <Label htmlFor="cointegrated-only">Mostrar apenas pares cointegrados</Label>
                              </div>
                            )}
                          </div>
                        </AccordionContent>
