...
def save_all_fundamental_data_to_supabase():
    ...
    for ticker in NYSE_tickers:
        ...
        if ticker_data:
            ...
            ticker_data['exchange'] = 'NYSE'  # Exchange identifier
    ...
    try:
        ...
        if response:
            print("All NYSE recommendations data saved to Supabase 'recommendations_records' table in CSV format.")
            print("Data processing and storage complete.")
        else:
            print("Failed to save NYSE recommendations to Supabase.")
            ...
            csv_file_path = os.path.join(export_dir, f"all_USA_NYSE_{timestamp_str}.csv")
            
            df.to_csv(csv_file_path, index=False)
            print(f"NYSE recommendations data saved to local CSV file as backup: {csv_file_path}")

    except Exception as e:
        print(f"Error saving NYSE recommendations data to Supabase: {e}")
        # Backup to local CSV file
        try:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            export_dir = os.path.join(current_dir, "export")
            os.makedirs(export_dir, exist_ok=True)
            timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
            csv_file_path = os.path.join(export_dir, f"all_USA_NYSE.csv")
