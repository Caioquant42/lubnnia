"""
Collar UI Single Structure Module

Analyzes a single Collar UI structure using Moving Block Bootstrap (MBB).
Based on mbb_bootstrap_paths_demo.py script.
"""

from typing import Dict, Tuple, Any
from datetime import datetime, timedelta
import numpy as np

from .mbb import MBBCore, DataGatherer


def business_days_between(start_date: datetime, end_date: datetime) -> int:
    """Calcula o número de dias úteis entre duas datas."""
    return int(np.busday_count(start_date.date(), end_date.date()))


class CollarUISingle:
    """
    Analyzes a single Collar UI structure using Moving Block Bootstrap.
    """

    def __init__(self, n_bootstrap: int = 1000):
        self.n_bootstrap = n_bootstrap
        self.mbb_core = MBBCore()
        self.data_gatherer = DataGatherer(use_monte_carlo_selection=False)

    def get_price_series(self, ticker: str, dias_uteis: int) -> np.ndarray:
        """Baixa a série de preços de fechamento do ativo usando DataGatherer."""
        # Calcula período necessário: pelo menos 3x o horizonte ou horizonte + 20 dias
        periodo_dias = max(dias_uteis * 3, dias_uteis + 20)
        
        ticker_sa = ticker if ticker.endswith(".SA") else f"{ticker}.SA"
        
        # Se o período for muito longo (> 2 anos), usa start_date/end_date
        if periodo_dias > 500:
            end_date = datetime.now()
            start_date = end_date - timedelta(days=int(periodo_dias * 1.5))
            df = self.data_gatherer.get_data(
                asset_list=[ticker_sa],
                start_date=start_date.strftime("%Y-%m-%d"),
                end_date=end_date.strftime("%Y-%m-%d")
            )
        else:
            period_str = f"{periodo_dias}d"
            df = self.data_gatherer.get_data(asset_list=[ticker_sa], period=period_str)
        
        if df.empty or ticker_sa not in df.columns:
            raise ValueError(f"Não foi possível baixar dados para o ativo {ticker}.")
        
        col_name = df.columns[0]
        prices = df[col_name].values.astype(float)
        
        # Verifica se há dados suficientes
        if len(prices) < dias_uteis:
            if len(prices) < 20:
                raise ValueError(
                    f"ERRO: Número de observações ({len(prices)}) é muito pequeno. "
                    f"Necessário pelo menos {dias_uteis} dias úteis."
                )
        
        return prices

    def generate_mbb_paths(
        self,
        prices: np.ndarray,
        S0: float,
        dias_uteis: int,
        n_caminhos: int,
    ) -> np.ndarray:
        """Gera caminhos de preço usando Moving Block Bootstrap (MBB)."""
        # Retornos logarítmicos: log(P_t / P_{t-1}) = log(P_t) - log(P_{t-1})
        returns = np.diff(np.log(prices))
        
        bootstrap_samples = self.mbb_core.moving_block_bootstrap(
            returns,
            n_bootstrap=n_caminhos,
            sample_size=dias_uteis,
        )
        
        # Constrói os caminhos de preço usando retornos logarítmicos: P_t = P_0 * exp(sum(log_returns))
        paths_sem_S0 = S0 * np.exp(np.cumsum(bootstrap_samples, axis=1))
        paths = np.zeros((n_caminhos, dias_uteis + 1), dtype=float)
        paths[:, 0] = S0
        paths[:, 1:] = paths_sem_S0
        
        return paths

    def calculate_collar_ui_payoffs(
        self,
        paths: np.ndarray,
        estrutura_params: Dict,
    ) -> Tuple[np.ndarray, np.ndarray, Dict]:
        """
        Calcula os payoffs da estrutura COLLAR UI para cada caminho.
        
        Returns:
            payoffs: Payoffs em % para cada caminho
            cenarios: Array indicando o cenário (0=perda, 1=ganho_sem_barreira, 2=ganho_com_barreira)
            stats: Estatísticas dos cenários
        """
        S0 = estrutura_params["S0"]
        strike_put = estrutura_params["strike_put"]
        barreira_ativacao = estrutura_params["barreira_ativacao"]
        ganho_max_ativado = estrutura_params["ganho_max_ativado"]
        ganho_max_nao_ativado = estrutura_params["ganho_max_nao_ativado"]
        prejuizo_maximo = estrutura_params["prejuizo_maximo"]
        
        n_caminhos, horizonte = paths.shape
        payoffs = np.zeros(n_caminhos)
        cenarios = np.zeros(n_caminhos, dtype=int)
        
        barreira_abs = S0 * barreira_ativacao
        
        for i in range(n_caminhos):
            path = paths[i, :]
            preco_final = path[-1]
            
            barreira_atingida = np.any(path >= barreira_abs)
            retorno = (preco_final - S0) / S0
            
            if preco_final < S0:
                payoff = max(retorno, -prejuizo_maximo)
                cenarios[i] = 0
            else:
                if barreira_atingida:
                    payoff = min(retorno, ganho_max_ativado)
                    cenarios[i] = 2
                else:
                    payoff = min(retorno, ganho_max_nao_ativado)
                    cenarios[i] = 1
            
            payoffs[i] = payoff
        
        stats = {
            "n_perda": int(np.sum(cenarios == 0)),
            "n_ganho_sem_barreira": int(np.sum(cenarios == 1)),
            "n_ganho_com_barreira": int(np.sum(cenarios == 2)),
            "pct_perda": float(np.sum(cenarios == 0) / n_caminhos * 100),
            "pct_ganho_sem_barreira": float(np.sum(cenarios == 1) / n_caminhos * 100),
            "pct_ganho_com_barreira": float(np.sum(cenarios == 2) / n_caminhos * 100),
            "payoff_medio": float(np.mean(payoffs)),
            "payoff_mediano": float(np.median(payoffs)),
            "payoff_std": float(np.std(payoffs)),
            "payoff_min": float(np.min(payoffs)),
            "payoff_max": float(np.max(payoffs)),
        }
        
        return payoffs, cenarios, stats

    def get_single_structure(
        self,
        ticker: str,
        estrutura_params: Dict,
        n_caminhos: int,
    ) -> Dict[str, Any]:
        """
        Analisa uma única estrutura COLLAR UI.
        
        Args:
            ticker: Ticker do ativo
            estrutura_params: Parâmetros da estrutura (S0, strike_put, strike_call, etc.)
            n_caminhos: Número de caminhos de bootstrap
        
        Returns:
            Dict com todos os dados da estrutura
        """
        S0 = estrutura_params["S0"]
        dias_uteis = estrutura_params["dias_uteis"]
        prices = self.get_price_series(ticker, dias_uteis)
        paths = self.generate_mbb_paths(prices, S0, dias_uteis, n_caminhos)
        payoffs, cenarios, stats = self.calculate_collar_ui_payoffs(paths, estrutura_params)
        
        # Calculate additional statistics
        percentis = [5, 25, 50, 75, 95]
        percentis_dict = {p: float(np.percentile(payoffs, p)) for p in percentis}
        
        # VaR and CVaR
        VaR_5 = float(np.percentile(payoffs, 5))
        CVaR_5 = float(np.mean(payoffs[payoffs <= VaR_5])) if np.any(payoffs <= VaR_5) else VaR_5
        
        # Sharpe Ratio (assuming risk-free rate = 0)
        expected_return = float(np.mean(payoffs))
        std = float(np.std(payoffs))
        sharpe_ratio = float(expected_return / std) if std > 0 else 0.0
        
        # Sortino Ratio
        downside_std = float(np.std(payoffs[payoffs < 0])) if np.any(payoffs < 0) else 0.0
        sortino_ratio = float(expected_return / downside_std) if downside_std > 0 else 0.0
        
        # Probability of positive gain
        prob_ganho_positivo = float(np.sum(payoffs > 0) / len(payoffs))
        
        # Conditional expected gain
        ganho_esperado_condicional = float(np.mean(payoffs[payoffs > 0])) if np.any(payoffs > 0) else 0.0
        
        # Probability of maximum loss
        prob_perda_max = float(np.sum(payoffs <= -estrutura_params['prejuizo_maximo']) / len(payoffs))
        
        # Probability of maximum gains
        prob_ganho_max_ativado = float(np.sum(payoffs >= estrutura_params['ganho_max_ativado']) / len(payoffs))
        prob_ganho_max_nao_ativado = float(
            np.sum((payoffs >= estrutura_params['ganho_max_nao_ativado']) & (cenarios == 1)) / len(payoffs)
        )
        
        return {
            "ticker": ticker,
            "params": estrutura_params,
            "statistics": stats,
            "payoffs": payoffs.tolist(),
            "scenarios": cenarios.tolist(),
            "paths": paths.tolist(),
            "additional_metrics": {
                "expected_return": expected_return,
                "std": std,
                "sharpe_ratio": sharpe_ratio,
                "sortino_ratio": sortino_ratio,
                "prob_ganho_positivo": prob_ganho_positivo,
                "ganho_esperado_condicional": ganho_esperado_condicional,
                "VaR_5": VaR_5,
                "CVaR_5": CVaR_5,
                "percentis": percentis_dict,
                "prob_perda_max": prob_perda_max,
                "prob_ganho_max_ativado": prob_ganho_max_ativado,
                "prob_ganho_max_nao_ativado": prob_ganho_max_nao_ativado,
            },
        }


def get_collar_ui_single(
    ticker: str,
    S0: float,
    strike_put_pct: float,
    strike_call_pct: float,
    expiration_date: str,
    barrier_pct: float,
    n_bootstrap: int = 1000,
) -> Dict[str, Any]:
    """
    Função helper para análise de uma única estrutura COLLAR UI.
    
    Args:
        ticker: Ticker do ativo
        S0: Preço inicial do ativo
        strike_put_pct: Strike Put em % do S0 (ex: 90.0 para 90%)
        strike_call_pct: Strike Call em % do S0 (ex: 107.5 para 107.5%)
        expiration_date: Data de vencimento (DD-MM-YYYY)
        barrier_pct: Barreira de ativação Up&In em % do S0 (ex: 144.0 para 144%)
        n_bootstrap: Número de caminhos de bootstrap
    
    Returns:
        Dict com dados da estrutura
    """
    # Parse expiration date
    data_venc = datetime.strptime(expiration_date, "%d-%m-%Y")
    data_atual = datetime.now()
    
    if data_venc <= data_atual:
        raise ValueError("Data de vencimento deve ser futura.")
    
    # Calcula dias úteis
    dias_uteis = business_days_between(data_atual, data_venc)
    
    if dias_uteis <= 0:
        raise ValueError("Data de vencimento muito próxima ou no passado.")
    
    # Converte percentuais para decimais
    strike_put = strike_put_pct / 100.0
    strike_call = strike_call_pct / 100.0
    barreira_ativacao = barrier_pct / 100.0
    
    # Calcula ganhos máximos
    ganho_max_ativado = strike_call - 1.0
    ganho_max_nao_ativado = barreira_ativacao - 1.0 - 0.0001
    prejuizo_maximo = 1.0 - strike_put
    
    estrutura_params = {
        "S0": S0,
        "strike_put": strike_put,
        "strike_call": strike_call,
        "data_vencimento": data_venc.isoformat(),
        "dias_uteis": dias_uteis,
        "barreira_ativacao": barreira_ativacao,
        "ganho_max_ativado": ganho_max_ativado,
        "ganho_max_nao_ativado": ganho_max_nao_ativado,
        "prejuizo_maximo": prejuizo_maximo,
    }
    
    analyzer = CollarUISingle(n_bootstrap=n_bootstrap)
    return analyzer.get_single_structure(
        ticker, estrutura_params, n_bootstrap
    )



