...
- **Pure Python**: Implemented entirely in Python/NumPy - no compilation required
...
No compilation needed! The module is implemented entirely in Python using NumPy for performance.
...
- **Implementation**: Pure Python using NumPy for optimized numerical computations
- **Random Number Generation**: NumPy's default_rng for reproducibility
- **Random Seed**: 1987 (default) for reproducibility
- **Time Complexity**: O(n_bootstrap × sample_size × iterations)
- **Platform Support**: Cross-platform (Windows, Linux, macOS) - no compilation needed
