...
Utiliza o OPLAB Client como fonte de dados históricos.

Uso esperado:
    python mbb_bootstrap_paths_comparison_oplab.py
"""
...
def get_price_series(ticker: str, dias_uteis: int) -> np.ndarray:
    """
    Baixa a série de preços de fechamento do ativo usando OPLAB Client.
    
    Args:
        ticker: Símbolo do ativo (ex: 'PETR4')
        dias_uteis: Número de dias úteis necessários
        
    Returns:
        numpy array com os preços de fechamento em ordem cronológica (mais antigo primeiro)
    """
    # Calcula o período necessário (similar ao DataGatherer)
    # Precisa de dados históricos suficientes para o bootstrap
    periodo_dias = max(dias_uteis * 3, dias_uteis + 20)
    
    # Calcula as datas: from_date é periodo_dias dias úteis antes de hoje
    # Para garantir dados suficientes, vamos buscar um período maior
    to_date = datetime.now()
    # Adiciona um buffer de alguns dias para garantir dados suficientes
    # Considerando que há aproximadamente 21 dias úteis por mês
    days_back = int(periodo_dias * 1.5)  # Buffer de 50% para garantir dias úteis suficientes
    from_date = to_date - timedelta(days=days_back)
    
    # Formata as datas no formato ISO 8601 requerido pela API
    from_date_str = from_date.strftime('%Y-%m-%dT00:00:00Z')
    to_date_str = to_date.strftime('%Y-%m-%dT23:59:59Z')
    
    try:
        # Busca dados históricos via OPLAB API
        hist_data = _oplab_client.market.historical.get_historical_data(
            symbol=ticker,
            resolution='1d',  # Dados diários
            from_date=from_date_str,
            to_date=to_date_str,
            smooth=True,  # Preenche valores zerados com o dia anterior
            df='timestamp'  # Formato timestamp para facilitar ordenação
        )
        
        if not hist_data or 'data' not in hist_data or not hist_data['data']:
            print(f"Não foi possível baixar dados para o ativo {ticker}.")
            print(f"Resposta da API: {hist_data}")
            sys.exit(1)
        
        # Extrai os dados e ordena por timestamp (cronologicamente)
        data_array = hist_data['data']
        
        # Converte para lista de tuplas (timestamp, close) e ordena
        prices_with_timestamp = [
            (item.get('time', 0), item.get('close', 0.0))
            for item in data_array
            if item.get('close') is not None and item.get('close') > 0
        ]
        
        if not prices_with_timestamp:
            print(f"Nenhum dado de preço válido encontrado para {ticker}.")
            sys.exit(1)
        
        # Ordena por timestamp (mais antigo primeiro)
        prices_with_timestamp.sort(key=lambda x: x[0])
        
        # Extrai apenas os preços de fechamento
        prices = np.array([price for _, price in prices_with_timestamp], dtype=float)
        ...
        if len(prices) < dias_uteis:
            if len(prices) < 20:
                ...
                print(f"Período solicitado: {from_date_str} a {to_date_str}")
                sys.exit(1)
            else:
                print(
        ...
        print(f"Erro ao buscar dados do OPLAB para {ticker}: {str(e)}")
        print(f"Verifique se o ticker está correto e se você tem acesso à API OPLAB.")
        sys.exit(1)
