...
def add_sa_to_tickers(tickers):
    return [ticker + '=X' for ticker in tickers]
...
def fetch_and_save_data(ticker_symbol, interval):
    ...
    try:
        ...
        historical_data = stock_data.history(period="max", interval=interval, auto_adjust=False)
        ...
        historical_data.insert(1, 'Symbol', ticker_symbol.replace('=X', ''))
...
def load_data_to_dolphindb(data, symbol, interval):
    ...
    The table name convention is: <interval>
    """
    try:
        # Upload the data to the server
        s.upload({'data': data})
        table_name = f"currency_{interval}"
...
def remove_duplicates(table_name):
    ...
    def removeDuplicates() {{
        currency = loadTable("dfs://yfs", "{table_name}")
        
        // Find duplicates based on Datetime and Symbol
        duplicates = select Datetime, Symbol, count(*) as cnt from currency 
...
if __name__ == "__main__":
    ...
    # Get ticker symbols from the dict (using the "CURRENCY" key as in your example)
    ticker_symbols = TICKERS_DICT["CURRENCY"]
